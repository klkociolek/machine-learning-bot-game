...
def setup_model(dataset, batch_size=64, num_workers=1, load_model=False, load_path=ConfigParams.model_path.value):
    ...
    #     spatial_input_shape=spatial_obs_space,
    #     num_non_spatial_features=non_spatial_obs_space,
    #     action_dim=len(action_mask)
    # )
    #     model = TransformerPolicy2(
    #         spatial_input_shape=spatial_obs_space,
    #         num_non_spatial_features=non_spatial_obs_space,
    #         action_dim=len(action_mask)
    #     )
        model = TransformerPolicy3(
            spatial_input_shape=spatial_obs_space,
            num_non_spatial_features=non_spatial_obs_space,
            action_dim=len(action_mask)
        )
        ...
        try:
            if dataloader_train is not None:
                for data in tqdm(dataloader_train):
                    ...
                    optimizer.zero_grad()
                    _, action_log_probs = model.get_action_log_probs(spatial_obs, non_spatial_obs, action_mask)  # , action_mask)
...
if __name__ == '__main__':
    ...
    try:
        training_losses, validation_losses = train(model, device, dataloader_train, dataloader_valid, n_epochs=5,
