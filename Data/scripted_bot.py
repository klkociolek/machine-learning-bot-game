...
import botbowl
from botbowl import Action, ActionType, Square, BBDieResult, Skill, ProcBot, Game, EnvConf, BotBowlEnv
...
class ScriptedBot(ProcBot):
    ...
    def __init__(self, name, out_path=".\scripted_dataset", dump=True):
        super(ScriptedBot, self).__init__(name)
        self.dump = dump
        ...
        # Formations
        self.off_formation = get_wedge_offense()
        self.def_formation = get_zone_defense()
    ...
    def _make_plan(self, game: botbowl.Game, ball_carrier):
        ...
        if attacker is not None and p_self_up > 0.94 and block_p_fumble_self == 0:
            ...
            if self.logging_enabled:
                print(
                    f"Safe block with {attacker.role.name} -> {defender.role.name}, p_self_up={p_self_up}, p_opp_down={p_opp_down}")
            return
        ...
        if attacker is not None and (p_opp_down > (1 - p_self_up) or block_p_fumble_opp > 0):
            ...
            if self.logging_enabled:
                print(
                    f"Block with {player.role.name} -> {defender.role.name}, p_self_up={p_self_up}, p_opp_down={p_opp_down}")
            return
    ...
    def block(self, game):
        ...
        if ActionType.SELECT_BOTH_DOWN in actions and game.get_ball_carrier() == defender and not defender.has_skill(
                Skill.BLOCK):
            return Action(ActionType.SELECT_BOTH_DOWN)
    ...
    def register_bot():
        ...
        """
        if ScriptedBot.BOT_ID.lower() not in botbowl.list_bots():
            botbowl.register_bot(ScriptedBot.BOT_ID, ScriptedBot)


def path_to_move_actions(game: botbowl.Game, player: botbowl.Player, path: Path, do_assertions=True) -> List[
    Action]:
    """
